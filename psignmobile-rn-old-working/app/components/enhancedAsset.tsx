...
Asset.prototype.downloadAsyncWithoutHash = async function({ cache }) {
  ...
  const path = `file://${NativeModules.ExponentFileSystem.documentDirectory}/${this.name}.${this.type}`;
  ...
  try {
    let exists, uri;
    ({ exists, uri } = await NativeModules.ExponentFileSystem.getInfoAsync(path, {
      //cache
    ...
    if (!exists) {
      ...
      ({ uri } = await NativeModules.ExponentFileSystem.downloadAsync(this.uri, path, {
        //cache
