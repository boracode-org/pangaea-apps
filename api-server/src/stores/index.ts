...
import { CurrencyStore } from "./scheduler";
 ...
 */
export type ISerializedState = { [P in keyof IStores]?: IStores[P]["state"] };
...
export class Store {
  ...
  constructor(state?: ISerializedState) {
    ...
    this.domains = {
      ...
      scheduler: new CurrencyStore(state && state.scheduler),
      router: new RouterStore(state && state.router),
      appStore: new AppStore(state && state.appStore)
    };
  }
