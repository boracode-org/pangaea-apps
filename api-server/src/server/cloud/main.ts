...
Parse.Cloud.afterSave("Media", async (request) => {
    ...
    if (!processed || processedDate < updatedAt) // only process new files
    {
        ...
        var filename = video.get("file") ? (video.get("file") as Parse.File).name() : "";
            ...
            ffmpeg(filePath)
                ...
                .on("end", async () => {
                    ...
                    video.set("video_thumbnail", Parse.File.fromJSON(
                        { __type: "File", name: props.thumbnail, url: "http://" + props.thumbnail }));
